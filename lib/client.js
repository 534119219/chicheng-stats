...
.dshs-host[data-collapsed="true"]{display:flex;justify-content:center;margin:0 0 8px}
...
.dshs-setcard{box-sizing:border-box;width:calc(100% - 4px);margin:0 2px 8px;display:block;text-align:left;
...
`;
		...
		/**
		 * Resolve {anchor, before} for the widget; null when the sidebar is
		 * not ready. `position` ("above" | "below") selects which side of the
		 * Settings seat (the shell's sidebar.settings slot host) the widget
		 * mounts on — every viewport (desktop, collapsed rail, mobile drawer).
		 */
		...
		function SettingsCard({ cardSettings, data, error, onOpen }) {
			...
			const padding = card.size === "large" ? "12px 14px" : card.size === "medium" ? "10px 12px" : "8px 10px";
			...
			return h("button", {
				...
				style: { gridTemplateColumns: `repeat(${columns}, 1fr)`, rowGap: `${gap}px`, columnGap: `${gap * 2}px` },
			},
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				// attribute (rail alignment) and re-renders together.
				const setCollapsed = (next) => {
					if (next === collapsed) return;
					collapsed = next;
					if (host !== null) host.dataset.collapsed = String(next);
					render();
					requestAnimationFrame(alignRailIcon);
				};

				// Rail mode: shift the icon horizontally so its center sits
				// exactly on the settings trigger's center (measured from the
				// live DOM — independent of any CSS layout quirk).
				// `appliedDx` remembers our own transform so measurements use
				// the UNTRANSFORMED position; otherwise the correction feeds
				// back into the next measurement and the icon oscillates.
				let appliedDx = 0;
				const alignRailIcon = () => {
					if (host === null) return;
					if (collapsed !== true) {
					...
					const seat = document.querySelector('[data-slot="sidebar.settings"]');
					const trigger = seat !== null ? seat.querySelector("button") : findSettingsButton();
					const iconEl = host.querySelector(".dshs-rail");
					if (trigger === null || iconEl === null) return;
					const tr = trigger.getBoundingClientRect();
					const ir = iconEl.getBoundingClientRect();
					if (tr.width === 0 || ir.width === 0) return;
					const triggerCenter = tr.left + tr.width / 2;
					const iconCenter = ir.left - appliedDx + ir.width / 2;
					const dx = triggerCenter - iconCenter;
					if (Math.abs(dx) < 0.5) {
						if (appliedDx !== 0) {
							appliedDx = 0;
							host.style.transform = "";
					...
					appliedDx = dx;
					host.style.transform = `translateX(${dx.toFixed(2)}px)`;
				};
				...
				probeTimer = setInterval(() => {
					...
					// Rail alignment self-corrects every tick too (cheap).
					alignRailIcon();
				}, 400);
