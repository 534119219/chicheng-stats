...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
/* Theme-aware tokens: light follows the dsh theme; dark keeps the classic
   dark palette so the dark appearance is unchanged. */
:root{
  --dshs-surface-1:var(--dsw-alias-bg-layer-1);
  --dshs-surface-2:var(--dsw-alias-bg-layer-2);
  --dshs-surface-input:var(--dsw-alias-bg-layer-1);
  --dshs-card-bg:var(--dsw-alias-bg-layer-3);
  --dshs-accent:var(--dsw-alias-button-primary-fill);
  --dshs-accent-foreground:var(--dsw-alias-label-primary-foreground);
  --dshs-focus:var(--dsw-alias-button-primary-fill);
  --dshs-error:var(--dsw-alias-state-error-primary);
  --dshs-success:var(--dsw-alias-state-success-primary);
  --dshs-mask:var(--dsw-alias-bg-mask-1)
}
body[data-ds-dark-theme]{
  --dshs-surface-1:#10141d;
  --dshs-surface-2:#1b202b;
  --dshs-surface-input:#10141d;
  --dshs-card-bg:#43454a;
  --dshs-accent:#3964fe;
...
.dshs-setcard{box-sizing:border-box;width:100%;margin:0;display:block;text-align:left;
  border:1px solid var(--dsw-alias-border-l2,#3a3f4b);background:var(--dshs-card-bg);
...
.dshs-mask{position:fixed;inset:0;z-index:2147482000;background:var(--dshs-mask,rgba(8,10,14,.55));backdrop-filter:blur(2px);
...
.dshs-seg{display:flex;gap:4px;padding:3px;border:1px solid var(--dsw-alias-border-l2,#3a3f4b);border-radius:10px;
  background:var(--dshs-surface-2,#1b202b)}
...
.dshs-chip{box-sizing:border-box;padding:8px 10px;border:1px solid var(--dsw-alias-border-l1,#262b36);border-radius:10px;
  background:var(--dshs-surface-2,#1b202b);min-width:0}
...
.dshs-model-bar{height:6px;border-radius:99px;background:var(--dshs-surface-2,#1b202b);overflow:hidden}
.dshs-model-bar-fill{height:100%;border-radius:99px;background:var(--dshs-accent,#3964fe)}
.dshs-chart{border:1px solid var(--dsw-alias-border-l1,#262b36);border-radius:10px;background:var(--dshs-surface-2,#1b202b);padding:8px}
...
.dshs-table th{position:sticky;top:0;background:var(--dshs-surface-2,#1b202b);color:var(--dsw-alias-label-secondary,#9aa3b2);
...
.dshs-filtlabel{font-size:12px;color:var(--dsw-alias-label-secondary,#9aa3b2);flex:none;width:72px;white-space:nowrap}
.dshs-filtrow .dshs-seg{flex-wrap:wrap}
.dshs-error{color:var(--dshs-error,#f45d5d);font-size:12px;padding:12px;text-align:center}
...
.dshs-tip{position:fixed;z-index:2147482100;width:240px;box-sizing:border-box;padding:10px 12px;
  border:1px solid var(--dsw-alias-border-l2,#3a3f4b);border-radius:12px;background:var(--dshs-surface-2,#1b202b);
  color:var(--dsw-alias-label-primary,#e6e9ef);font-size:12px;line-height:1.5;box-shadow:0 12px 32px rgba(0,0,0,.45);
  pointer-events:auto}
.dshs-tip-title{font-weight:600;font-size:12px;margin-bottom:6px;color:var(--dsw-alias-label-primary,#e6e9ef)}
.dshs-tip-row{display:flex;justify-content:space-between;gap:10px;margin-top:3px;align-items:baseline}
.dshs-tip-label{color:var(--dsw-alias-label-secondary,#9aa3b2);flex:none;white-space:nowrap}
...
.dshs-tip-error{color:var(--dshs-error,#f45d5d)}
.dshs-tip-footer{display:flex;justify-content:flex-end;margin-top:8px}
.dshs-tip-refresh{border:1px solid var(--dsw-alias-border-l2,#3a3f4b);background:var(--dsw-alias-button-elevated-fill,#1f2430);
  color:var(--dsw-alias-label-primary,#e6e9ef);font-size:11px;padding:2px 8px;border-radius:7px;cursor:pointer}
.dshs-tip-refresh:hover{background:var(--dsw-alias-button-floating-hover,#262c3a)}
...
.dshs-set-saved{font-size:12px;color:var(--dshs-success,#3ddc84)}
...
.dshs-set-textarea{box-sizing:border-box;width:100%;padding:8px 10px;font-size:13px;color:inherit;
  background:var(--dshs-surface-input,#10141d);border:1px solid var(--dsw-alias-border-l2,#3a3f4b);border-radius:9px;outline:none;font-family:inherit;resize:vertical}
.dshs-set-textarea:focus{border-color:var(--dshs-focus,#639efe)}
.dshs-set-ph{display:flex;gap:6px;flex-wrap:wrap}
.dshs-set-ph-chip{border:1px solid var(--dsw-alias-border-l2,#3a3f4b);background:var(--dshs-surface-2,#1b202b);
  color:var(--dsw-alias-label-secondary,#9aa3b2);font-size:11px;padding:3px 9px;border-radius:99px;cursor:pointer}
.dshs-set-ph-chip:hover{border-color:var(--dshs-focus,#639efe);color:var(--dsw-alias-label-primary,#e6e9ef)}
.dshs-set-input{box-sizing:border-box;width:80px;padding:5px 8px;font-size:13px;color:inherit;
  background:var(--dshs-surface-input,#10141d);border:1px solid var(--dsw-alias-border-l2,#3a3f4b);border-radius:8px;outline:none;font-family:inherit}
...
.dshs-set-color{box-sizing:border-box;width:38px;height:28px;padding:2px;border:1px solid var(--dsw-alias-border-l2,#3a3f4b);
  border-radius:8px;background:var(--dshs-surface-input,#10141d);cursor:pointer}
...
.dshs-set-preview{border:1px dashed var(--dsw-alias-border-l2,#3a3f4b);border-radius:10px;padding:10px;background:var(--dshs-surface-2,#1b202b)}
...
`;
		...
		function TextLine({ textSettings, data, error, onOpen, onMouseEnter, onMouseLeave }) {
			...
			if (ts.background) {
				style.background = ts.bgColor ?? "var(--dshs-surface-2,#1b202b)";
			...
			return h("div", rootProps,
				...
				breakdown !== null && breakdown !== undefined
					? h("div", { className: "dshs-tip-hint" },
						`缓存命中 ${formatTokens(breakdown.cacheHit ?? 0)} / 缓存未命中 ${formatTokens(breakdown.cacheMiss ?? 0)} / 输出 ${formatTokens(breakdown.output ?? 0)}`)
					: null,
				...
				data.updatedAt !== null && data.updatedAt !== undefined
					? h("div", { className: "dshs-tip-hint" }, `${t("dsUpdated")}: ${new Date(data.updatedAt).toLocaleTimeString()}`)
					: null,
				footer,
			);
		}
		...
		function StatsWidget({ settings, collapsed, data, error, onOpen, deepseek, deepseekLoading, deepseekError, onFetchDeepseek }) {
			...
			const showTip = (event) => {
				...
				if (rect.width > 0 && rect.height > 0) setTip({ left: rect.left + rect.width / 2, top: rect.top });
			};
