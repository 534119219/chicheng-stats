		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const mountHost = () => {
					...
					// The class flip fires once at the START of the expand/collapse
					// animation, when the width has not moved yet — so also watch
					// the actual size: ResizeObserver fires through the animation
					// and settles on the final width, un-sticking the icon state.
					if (rootEl !== null && rootEl !== undefined && typeof ResizeObserver !== "undefined") {
						resizeObserver = new ResizeObserver(() => {
							if (disposed || host === null || anchorEl === null) return;
							const next = isCollapsed(anchorEl);
							if (next !== collapsed) {
								collapsed = next;
								render();
							}
