...
`;
		...
		function DeepseekTooltip({ data, loading, error, position }) {
			const style = tipStyle(position);
			if (loading) return h("div", { className: "dshs-tip", style }, t("loading"));
			if (error !== null && error !== undefined) {
				return h("div", { className: "dshs-tip dshs-tip-error", style }, `${t("dsTitle")} · ${error}`);
			}
			if (data === null || data === undefined) return null;
			if (!data.configured) {
				return h("div", { className: "dshs-tip", style },
			...
			return h("div", { className: "dshs-tip", style },
				...
				breakdown !== null && breakdown !== undefined
					? h("div", { className: "dshs-tip-hint" },
						`${t("dsBalanceBreakdown")} · 缓存命中 ${formatTokens(breakdown.cacheHit ?? 0)} / 缓存未命中 ${formatTokens(breakdown.cacheMiss ?? 0)} / 输出 ${formatTokens(breakdown.output ?? 0)}`)
					: null,
		...
		function StatsWidget({ settings, collapsed, data, error, onOpen, deepseek, deepseekLoading, deepseekError, onFetchDeepseek }) {
			...
			return h(React.Fragment, null,
				child,
				tip !== null && h(DeepseekTooltip, { data: deepseek, loading: deepseekLoading, error: deepseekError, position: tip }),
			);
		}
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const fetchDeepseek = async () => {
					...
					render();
					try {
						deepseekData = await call("deepseek");
						deepseekError = null;
					} catch (e) {
