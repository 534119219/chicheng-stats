...
.dshs-panel .dshs-trend-tip{
  position:absolute;
  transform:translate(-50%,-110%);
  background:var(--dshs-surface-1,#ffffff);
  border:1px solid var(--dsw-alias-border-l2,#d7dbe3);
  border-radius:8px;
...
.dshs-panel .dshs-trend-tip-row{display:flex;justify-content:space-between;gap:12px}
.dshs-panel .dshs-trend-tip-row span:first-child{color:var(--dsw-alias-label-secondary,#667085)}
.dshs-panel .dshs-trend-tip-row span:last-child{color:var(--dsw-alias-label-primary,#1f2430);font-weight:600;font-variant-numeric:tabular-nums}
...
`;
		...
		function TrendChart({ trend }) {
			...
			const labels = trend.map((b, i) =>
				i % step === 0
					...
					: null);
			const tipRows = hover === null ? null : [
				{ label: t("colInput"), value: formatTokens(hover.b.input) },
				{ label: t("colOutput"), value: formatTokens(hover.b.output) },
				{ label: t("cacheHit"), value: formatTokens(hover.b.cacheRead) },
				{ label: t("requests"), value: formatRequests(hover.b.requests) },
			];
			return h("div", { className: "dshs-chart" },
				h("svg", { viewBox: `0 0 ${W} ${H}`, width: "100%", role: "img", "aria-label": t("trend") },
					...
					yTicks.map((tick) => h("line", {
						key: `yline-${tick.frac}`,
						x1: padL,
						y1: tick.y,
						x2: W - padR,
						y2: tick.y,
						stroke: "var(--dsw-alias-border-l1,#d7dbe3)",
						strokeWidth: 1,
						strokeDasharray: "3 3",
					})),
					yTicks.map((tick) => h("text", {
						key: `ylabel-${tick.frac}`,
						x: padL - 6,
						y: tick.y + 3,
						fontSize: 9,
					...
					pts.map((p) => h("circle", {
						...
						stroke: "var(--dshs-surface-1,#ffffff)",
						strokeWidth: 1.5,
				...
				hover !== null && h("div", { className: "dshs-trend-tip", style: { left: `${(hover.x / W) * 100}%`, top: `${(hover.y / H) * 100}%` } },
					tipRows.map((row) => h("div", { key: row.label, className: "dshs-trend-tip-row" },
						h("span", {}, row.label),
						h("span", {}, row.value),
					)),
				),
		...
		function DonutChart({ items, label }) {
			...
			return h("div", { className: "dshs-donut" },
				h("svg", { viewBox: "0 0 160 160", width: 160, height: 160, role: "img", "aria-label": label ?? t("providerDist") }, segments),
						...
						(data?.providers ?? []).length > 0
							? h("div", { className: "dshs-duo" },
								h("section", { className: "dshs-section" },
									...
									h(DonutChart, { items: (data?.models ?? []).map((m) => ({ name: m.model, value: m.tokens })), label: t("modelDist") }),
								),
								h("section", { className: "dshs-section" },
									h("h3", null, t("providerDist")),
									h(ModelRows, { models: (data?.providers ?? []).map((p) => ({ model: p.name ?? p.provider, requests: p.requests, tokens: p.tokens })) }),
								),
							)
							: h("section", { className: "dshs-section" },
								h("h3", null, t("modelDist")),
