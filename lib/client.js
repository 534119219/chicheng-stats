...
.dshs-filtlabel{font-size:12px;color:var(--dsw-alias-label-secondary,#9aa3b2);flex:none}
...
@media (max-width: 720px){
  ...
  .dshs-head .dshs-seg{order:3;width:100%;justify-content:center;padding:3px 0}
						...
						h("div", { className: "dshs-filtrow" },
							...
							h("div", { className: "dshs-seg", role: "group" },
								...
								(data?.providers ?? []).map((p) => h("button", {
									key: p.provider,
									type: "button",
									title: `${p.provider} · ${p.requests} ${t("requests")} · ${formatTokens(p.tokens)} ${t("tokens")}`,
