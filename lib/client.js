	...
	factory: (require) => {
		...
		const t = (key) => {
			const zh = {
				...
				posBelow: "设置下方",
				template: "显示格式（支持占位符）",
				templateHint: "点击下方占位符插入到光标处；「换行」插入换行符；也可手输中文占位符（如 {总Token}）；未识别的会原样保留。",
				newline: "换行",
				fontSize: "文字大小",
				cardSize: "卡片大小",
				cardSizeSmall: "小",
			...
			const en = {
				...
				templateHint: "Click a placeholder to insert it at the cursor; \"New line\" inserts a line break; Chinese names (e.g. {总Token}) also work; unknown tokens stay as-is.",
				newline: "New line",
				fontSize: "Font size",
				cardSize: "Card size",
				cardSizeSmall: "Small",
...
`;
		...
		function StatsSettingsSection() {
			...
			const insertNewline = () => insertToken("\n");
