	...
	factory: (require) => {
		...
		const t = (key) => {
			const zh = {
				...
				posBelow: "设置下方",
				template: "显示格式（支持占位符）",
				templateHint: "点击下方占位符插入到光标处；未识别的占位符会原样保留。",
				fontSize: "文字大小",
				cardSize: "卡片大小",
				cardSizeSmall: "小",
				cardSizeMedium: "中",
			...
			const en = {
				...
				templateHint: "Click a placeholder to insert it at the cursor; unknown tokens stay as-is.",
				fontSize: "Font size",
				cardSize: "Card size",
				cardSizeSmall: "Small",
...
.dshs-setcard{box-sizing:border-box;width:calc(100% - 4px);margin:0 2px 8px;display:block;text-align:left;
  border:1px solid var(--dsw-alias-border-l2,#3a3f4b);background:var(--dsw-alias-bg-l2,#1b202b);
...
`;
		...
		function TextLine({ textSettings, data, error, onOpen }) {
			const template = textSettings?.template && textSettings.template.trim() !== ""
				? textSettings.template
				: DEFAULT_TEMPLATE;
			...
			return h("button", {
				type: "button",
				className: "dshs-line",
				style: { fontSize: `${textSettings?.fontSize ?? 11}px` },
				title,
		...
		function SettingsCard({ cardSettings, data, error, onOpen }) {
			...
			const radius = card.size === "large" ? 14 : card.size === "medium" ? 12 : 10;
			...
			return h("button", {
				...
				"aria-label": t("cardLabel"),
				onClick: onOpen,
			}, h("div", { className: "dshs-setcard-grid", style: { rowGap: `${gap}px`, columnGap: `${gap * 2}px` } },
				items.map((item) => h("div", { key: item.label, className: "dshs-setcard-item" },
					h("div", { className: "dshs-setcard-label", style: { fontSize: `${titleSize}px` } }, item.label),
					h("div", { className: "dshs-setcard-value", style: { fontSize: `${valueSize}px` } }, item.value),
				)),
			));
		}
		...
		function StatsSettingsSection() {
			...
			const insertPlaceholder = (key) => {
				...
				if (el !== null && el !== undefined) {
					...
					patch((prev) => ({ ...prev, text: { ...prev.text, template: template + `{${key}}` } }));
				}
			};
			...
			return h("div", { className: "dshs-set" },
				...
				s.mode === "text"
					? h("div", { className: "dshs-set-block" },
						...
						h("div", { className: "dshs-set-ph" },
							PLACEHOLDERS.map(([key, label]) => h("button", {
								key,
								type: "button",
				...
				h("div", { className: "dshs-set-block" },
					h("div", { className: "dshs-set-label" }, t("preview")),
