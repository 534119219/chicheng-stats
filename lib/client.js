...
.dshs-set-section{display:flex;flex-direction:column;gap:10px;padding:2px 2px 0}
.dshs-set-section-title{font-size:14px;font-weight:600;color:var(--dsw-alias-label-primary,#e6e9ef);
  padding-bottom:8px;border-bottom:1px solid var(--dsw-alias-border-l1,#262b36)}
...
`;
		...
		function TextLine({ textSettings, data, error, onOpen }) {
			...
			if (ts.background) {
				style.background = "var(--dsw-alias-bg-l2,#1b202b)";
		...
		function SettingsCard({ cardSettings, data, error, onOpen }) {
			...
			const items = (card.items && card.items.length > 0 ? card.items : ALL_CARD_ITEMS)
				.filter((key) => itemValues[key] !== undefined)
				.map((key) => ({ key, label: itemValues[key][0], value: itemValues[key][1] }));
			const style = {
				padding,
