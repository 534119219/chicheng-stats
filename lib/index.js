...
function readTokenFromLeveldbDir(leveldbDir) {
  return new Promise((resolve) => {
    ...
    try {
      db = levelup(leveldown(leveldbDir));
      // Attach an error listener BEFORE get(): an open failure (e.g. a live
      // browser holds the LOCK) otherwise emits an unhandled 'error' on the
      // LevelUP instance and crashes the host process.
      db.on("error", () => done(null));
      db.get(DEEPSEEK_STORAGE_KEY, (err, value) => {
        if (err) return done(null);
        const token = value == null ? null : value.toString().slice(1);
        done(typeof token === "string" && token.trim() !== "" ? token : null);
      });
    } catch {
      done(null);
    }
...
async function readPlatformTokenFromLeveldb(dir) {
  // Never open a live browser's leveldb through the raw package (the unhandled
  // error crashes the host). First try the original safely; on any failure
  // read from a temporary copy of the directory, which has no LOCK contention.
