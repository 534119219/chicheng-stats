...
function sanitizeSettings(input) {
  ...
  if (input.text && typeof input.text === "object") {
    if (typeof input.text.template === "string" && input.text.template.trim() !== "") {
      out.text.template = input.text.template.slice(0, 1000);
    }
    const fontSize = clampNumber(input.text.fontSize, 8, 24);
    if (fontSize !== null) out.text.fontSize = fontSize;
  }
  if (input.card && typeof input.card === "object") {
    if (input.card.size === "small" || input.card.size === "medium" || input.card.size === "large") out.card.size = input.card.size;
    const titleSize = clampNumber(input.card.titleSize, 8, 24);
    if (titleSize !== null) out.card.titleSize = titleSize;
    const valueSize = clampNumber(input.card.valueSize, 10, 32);
    if (valueSize !== null) out.card.valueSize = valueSize;
    const gap = clampNumber(input.card.gap, 0, 24);
