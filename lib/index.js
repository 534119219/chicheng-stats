...
import { mkdir, readFile, writeFile, rename, readdir, stat, mkdtemp, copyFile, rm } from "node:fs/promises";
...
const inject = ["webServer", "webRuntime", "credentials"];
...
/** session id -> latest known conversation title (from `session/title` events). */
const sessionTitles = new Map();
const SESSION_TITLES_PATH = join(DATA_ROOT, "session-titles.json");
let sessionTitlesDirtyTimer = null;
...
/** Provider a request/header snapshot was routed to, or null. */
function providerOfEvent(event) {
  if (event?.type !== "request/header") return null;
  const config = event?.data?.header?.config;
  if (config === null || typeof config !== "object") return null;
  if (typeof config.provider === "string" && config.provider !== "") return config.provider;
  return null;
}
...
/** Content-delta chunk types — the first one marks the time-to-first-byte. */
...
function createFold(initial = {}) {
  ...
  // Provider in force per session (from request/header snapshots).
  const sessionProviders = new Map();
  // Per-request detail records (bounded ring; persisted to a separate file).
  const records = [];
  // Per-step timing: step/start → first content chunk (TTFT) and → usage
  // sample (total duration). Keyed `${sessionId}|${turn}|${step}`; entries are
  // removed on step/end so the maps stay bounded by in-flight steps.
  const stepStarts = new Map();
  const stepFirstChunks = new Map();

  /** @returns true when the event contributed a usage sample. */
  function ingest(sessionId, event) {
    // Track the model and provider each request was built under.
    if (event?.type === "request/header") {
      const model = modelOfEvent(event);
      if (model !== null) sessionModels.set(sessionId, model);
      const provider = providerOfEvent(event);
      if (provider !== null) sessionProviders.set(sessionId, provider);
    }
    ...
    if (turn !== undefined && step !== undefined) {
      ...
      if (event?.type === "step/start") {
        stepStarts.set(stepKey, ts);
        stepFirstChunks.delete(stepKey);
      } else if (event?.type === "step/end") {
        stepStarts.delete(stepKey);
        stepFirstChunks.delete(stepKey);
      } else if (event?.type === "assistant/chunk" && !stepFirstChunks.has(stepKey)) {
        const chunk = event?.data?.chunk;
        if (chunk !== null && typeof chunk === "object" && CONTENT_CHUNK_TYPES.has(chunk.type)) {
          stepFirstChunks.set(stepKey, ts);
        }
      }
    }
    ...
    const stepKey = turn !== undefined && step !== undefined ? `${sessionId}|${turn}|${step}` : null;
    const start = stepKey !== null ? stepStarts.get(stepKey) : undefined;
    const firstChunk = stepKey !== null ? stepFirstChunks.get(stepKey) : undefined;
    const ttft = start !== undefined && firstChunk !== undefined ? firstChunk - start : null;
    const duration = start !== undefined ? ts - start : null;
    ...
    if (last !== undefined && last.turn === turn && last.step === step) {
      ...
      bucket.cacheRead = (bucket.cacheRead ?? 0) + cacheRead;
      bucket.cacheWrite = (bucket.cacheWrite ?? 0) + cacheWrite;
      records.push({ t: ts, model, provider, session: sessionId, turn, step, input, cacheRead, cacheWrite, output, ttft, duration });
...
function decodeSessionLog(buffer) {
  // DeepSeek Harness Desktop bundles a Node runtime whose zlib native zstd
  // binding can CHECK-trap (SIGTRAP) on valid streaming zstd frames without
  // Frame_Content_Size — the exact shape the harness session-log writer
  // emits. The trap happens on a uv callback path, so try/catch cannot
  // contain it. Skip history backfill on Electron/Desktop runtimes by
  // default; set CHICHENG_ENABLE_SCAN=1 to opt back in.
  if (typeof process.versions?.electron === "string" && process.env.CHICHENG_ENABLE_SCAN !== "1") {
    return [];
  }
...
async function loadSessionTitles() {
  try {
    const parsed = JSON.parse(await readFile(SESSION_TITLES_PATH, "utf8"));
    if (parsed !== null && typeof parsed === "object") {
      for (const [id, title] of Object.entries(parsed)) {
        if (typeof title === "string" && title.trim() !== "") sessionTitles.set(id, title);
      }
    }
  } catch {
    // first run
...
function scheduleSessionTitlesSave() {
  if (sessionTitlesDirtyTimer !== null) return;
  sessionTitlesDirtyTimer = setTimeout(() => {
    sessionTitlesDirtyTimer = null;
    void flushSessionTitles();
  }, 300);
}

async function flushSessionTitles() {
  try {
    await mkdir(DATA_ROOT, { recursive: true });
    const tmp = `${SESSION_TITLES_PATH}.tmp`;
    await writeFile(tmp, JSON.stringify(Object.fromEntries(sessionTitles)), "utf8");
    await rename(tmp, SESSION_TITLES_PATH);
  } catch (error) {
    console.warn(`[chicheng-stats] session title flush failed:`, error instanceof Error ? error.message : String(error));
  }
}

/** One-time full scan for conversation titles when no title cache exists yet. */
async function refreshSessionTitlesIfNeeded() {
  if (existsSync(SESSION_TITLES_PATH)) return;
      ...
      try {
        ...
        for (const event of events) {
          if (event?.type === "session/title" && typeof event?.data?.title === "string" && event.data.title.trim() !== "") {
            if (sessionTitles.get(sessionId) !== event.data.title) changed = true;
            sessionTitles.set(sessionId, event.data.title);
          }
        }
      } catch {
  ...
  if (changed) scheduleSessionTitlesSave();
}
...
/** Records file schema version; bump to force a historical rebuild. */
const RECORDS_VERSION = 3;

/** Load persisted detail records into the fold (bounded). Older schema
 * versions are ignored so the startup rebuild re-folds them with timings. */
async function loadRecords() {
  try {
    const raw = await readFile(RECORDS_PATH, "utf8");
    const parsed = JSON.parse(raw);
    const list = Array.isArray(parsed) || parsed?.version !== RECORDS_VERSION
      ? null
      : parsed?.records;
...
async function flushRecords() {
  try {
    await mkdir(DATA_ROOT, { recursive: true });
    const payload = JSON.stringify({ version: RECORDS_VERSION, records: fold.allRecords() });
...
const DEFAULT_SETTINGS = Object.freeze({
  ...
  card: {
    ...
    valueSize: 15,
    gap: 8,
    bg: null,          // null = theme surface (bg-layer-3), follows light/dark
    border: 1,         // 0-3 px
    borderColor: null,
    radius: 10,        // 0-24
    titleColor: null,
...
async function loadSettings() {
  try {
    ...
    // Migrate the pre-0.6.2 default card background (rgb 67,69,74) to the
    // theme surface so the card follows light/dark themes.
    if (widgetSettings.card?.bg === "#43454a") {
      widgetSettings.card.bg = null;
...
/** Static display-name fallbacks for provider ids with no live metadata. */
const PROVIDER_NAME_FALLBACKS = {
  unknown: "Unknown",
  deepseek: "DeepSeek",
  openai: "OpenAI",
  anthropic: "Anthropic",
...
/** Turn a kebab/snake-case provider id into a readable title ("opencode-go" -> "Opencode Go"). */
function prettyProviderId(id) {
  return id
    .split(/[-_]+/)
    .filter((part) => part !== "")
...
/**
 * Resolve a provider id to the human-facing display name DSH itself uses:
 * registered adapter metadata (name) first, then the configurable-provider
 * directory (displayName), then a static mapping, then a title-cased id.
 */
function providerNameOf(providerId) {
  const id = typeof providerId === "string" && providerId !== "" ? providerId : "unknown";
  try {
    const llm = currentCtx?.llm;
    if (llm && typeof llm.listProviders === "function") {
      const found = llm.listProviders().find((p) => p?.id === id);
      if (found && typeof found.name === "string" && found.name !== "") return found.name;
    }
    if (llm && typeof llm.listConfigurableProviders === "function") {
      const found = llm.listConfigurableProviders().find((entry) => entry?.provider === id);
      if (found && typeof found.displayName === "string" && found.displayName !== "") return found.displayName;
    }
  } catch {
    // Fall through to the static mapping when the llm service is unavailable.
  }
  return PROVIDER_NAME_FALLBACKS[id] ?? prettyProviderId(id);
}

// ---------------------------------------------------------------- DeepSeek platform

const DEEPSEEK_PUBLIC_BASE_URL = process.env.DEEPSEEK_BASE_URL ?? "https://api.deepseek.com";
const DEEPSEEK_BALANCE_PATH = "/user/balance";
const DEEPSEEK_PLATFORM_USAGE_BASE = "https://platform.deepseek.com/api/v0/usage/by_api_key";
...
const DEEPSEEK_CACHE_TTL_MS = 60 * 1000;
const DEEPSEEK_DAY_STATE_PATH = join(DATA_ROOT, "deepseek-day.json");

let deepseekCache = { at: 0, data: null };

/** DeepSeek platform dashboard uses Asia/Shanghai calendar days. */
function deepseekTimezoneDay() {
  ...
  return { day, start, end: start + 24 * 60 * 60, tz: 8 * 60 * 60 };
}
...
function pickBalanceInfo(body) {
  if (body === null || typeof body !== "object" || !Array.isArray(body.balance_infos)) return null;
  const infos = body.balance_infos;
  const entry = infos.find((it) => it?.currency === "CNY") ?? infos[0];
  if (entry === undefined) return null;
  return {
    currency: typeof entry.currency === "string" ? entry.currency : "CNY",
    total: toFiniteNumber(entry.total_balance),
    granted: toFiniteNumber(entry.granted_balance),
    toppedUp: toFiniteNumber(entry.topped_up_balance),
    isAvailable: body.is_available === true,
  };
}

async function fetchDeepseekBalance(apiKey) {
  const base = DEEPSEEK_PUBLIC_BASE_URL.replace(/\/+$/, "");
  const response = await fetch(`${base}${DEEPSEEK_BALANCE_PATH}`, {
    headers: {
  ...
  const info = pickBalanceInfo(body);
  if (info === null) throw new Error("DeepSeek 余额接口返回格式异常");
  return info;
}

function deepseekDashboardData(body) {
  ...
  if (body?.code !== 0 || data?.biz_code !== 0 || data?.biz_data == null || typeof data.biz_data !== "object") {
    ...
    throw new Error(`DeepSeek 平台用量接口错误 (code ${code})`);
  }
  return data.biz_data;
}

async function deepseekFetchJson(url, token) {
...
async function fetchDeepseekTodayOfficial(token) {
  const { start, end, tz } = deepseekTimezoneDay();
  const query = `start=${start}&end=${end}&tz=${tz}`;
  const [amountBody, costBody] = await Promise.all([
    deepseekFetchJson(`${DEEPSEEK_PLATFORM_USAGE_BASE}/amount?${query}`, token),
    deepseekFetchJson(`${DEEPSEEK_PLATFORM_USAGE_BASE}/cost?${query}`, token),
  ]);
  const amountData = deepseekDashboardData(amountBody);
  const costData = deepseekDashboardData(costBody);
  ...
  let output = 0;
  let cost = 0;
  const series = Array.isArray(amountData) ? (amountData[0]?.series ?? []) : (amountData.series ?? []);
  for (const s of series) {
    if (s === null || typeof s !== "object" || !Array.isArray(s.buckets)) continue;
    for (const bucket of s.buckets) {
      ...
      requests += toFiniteNumber(usage.REQUEST) || 0;
      cacheHit += toFiniteNumber(usage.PROMPT_CACHE_HIT_TOKEN) || 0;
      cacheMiss += toFiniteNumber(usage.PROMPT_CACHE_MISS_TOKEN) || 0;
      output += toFiniteNumber(usage.RESPONSE_TOKEN) || 0;
    }
  }

  const currencyEntries = Array.isArray(costData) ? costData : (Array.isArray(costData.data) ? costData.data : []);
  const costEntry = (Array.isArray(currencyEntries) ? currencyEntries.find((e) => e?.currency === "CNY") : null)
    ?? (Array.isArray(currencyEntries) ? currencyEntries[0] : null);
  if (costEntry && Array.isArray(costEntry.series)) {
    for (const s of costEntry.series) {
      if (s === null || typeof s !== "object" || !Array.isArray(s.buckets)) continue;
      for (const bucket of s.buckets) {
        if (bucket === null || typeof bucket !== "object") continue;
        cost += toFiniteNumber(bucket.cost) || 0;
      }
    }
  }
...
async function loadDeepseekDayState() {
  try {
    const parsed = JSON.parse(await readFile(DEEPSEEK_DAY_STATE_PATH, "utf8"));
    if (parsed !== null && typeof parsed === "object" && typeof parsed.date === "string"
      && typeof parsed.opening === "number" && typeof parsed.last === "number") return parsed;
  } catch {}
  return null;
}

async function saveDeepseekDayState(state) {
  try {
    await mkdir(DATA_ROOT, { recursive: true });
    const tmp = `${DEEPSEEK_DAY_STATE_PATH}.tmp`;
    await writeFile(tmp, JSON.stringify(state), "utf8");
    await rename(tmp, DEEPSEEK_DAY_STATE_PATH);
  } catch {}
}

/** Balance-delta estimate for today's spend (fallback when no platform token). */
async function estimateDeepseekTodayCost(balanceTotal) {
  if (!Number.isFinite(balanceTotal)) return null;
  const today = dayKey(Date.now());
  const stored = await loadDeepseekDayState();
  const opening = stored !== null && stored.date === today
    ? stored.opening
    : (stored !== null ? stored.last : balanceTotal);
  await saveDeepseekDayState({ date: today, opening, last: balanceTotal });
  const consumed = Math.max(0, opening - balanceTotal);
  return Math.round(consumed * 100) / 100;
}

async function fetchDeepseekPlatformData() {
  const ctx = currentCtx;
  const summary = fold.summary(Date.now());
  const apiHit = ctx?.credentials ? await ctx.credentials.resolve(DEEPSEEK_API_KEY_REF) : undefined;
  if (apiHit === undefined) {
    return {
      configured: false,
      ...
      todayRequests: null,
      todayBreakdown: null,
      localTodayTokens: summary.today.tokens,
      localTodayRequests: summary.today.requests,
      message: "未配置 DEEPSEEK_API_KEY（设置 → 模型 或 ~/.dsh/.credentials.yaml）",
  ...
  let balance = null;
  try {
    balance = await fetchDeepseekBalance(apiHit.value);
  } catch (error) {
    return {
      configured: true,
  ...
  const platformHit = ctx?.credentials ? await ctx.credentials.resolve(DEEPSEEK_PLATFORM_TOKEN_REF) : undefined;
  let todayCost = null;
  let todayCostSource = null;
  let todayTokens = null;
  let todayRequests = null;
  let todayBreakdown = null;
  if (platformHit !== undefined) {
    try {
      const official = await fetchDeepseekTodayOfficial(platformHit.value);
      todayCost = Math.round(official.cost * 100) / 100;
      todayCostSource = "official";
      todayTokens = official.tokens;
      todayRequests = official.requests;
      todayBreakdown = { cacheHit: official.cacheHit, cacheMiss: official.cacheMiss, output: official.output };
    } catch {
      // platform token missing/expired → fall through to estimate / local
    }
  }
  if (todayCostSource === null && Number.isFinite(balance.total)) {
    todayCost = await estimateDeepseekTodayCost(balance.total);
    todayCostSource = "estimate";
  }

  return {
    configured: true,
    apiKeyConfigured: true,
    platformTokenConfigured: platformHit !== undefined,
    balance,
    todayCost,
    todayCostSource,
    todayTokens,
...
// ---------------------------------------------------------------- DeepSeek platform token (browser import)

async function browserProfileLeveldbDirs() {
  const local = process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local");
  const roots = [
    join(local, "Google", "Chrome", "User Data"),
    join(local, "Microsoft", "Edge", "User Data"),
    join(local, "360ChromeX", "Chrome", "User Data"),
    join(local, "360Chrome", "Chrome", "User Data"),
    join("D:", "360ChromeX", "Chrome", "User Data"),
    join("D:", "360Chrome", "Chrome", "User Data"),
  ];
  const dirs = [];
  for (const root of roots) {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      if (entry.name !== "Default" && !/^Profile \d+$/.test(entry.name) && !entry.name.startsWith("user-")) continue;
      const dir = join(root, entry.name, "Local Storage", "leveldb");
...
const DEEPSEEK_STORAGE_KEY = Buffer.from("_https://platform.deepseek.com\u0000\u0001userToken");

/** Chromium localStorage stores `userToken` as a JSON object string like `{"value":"..."}`. */
function decodeLocalStorageValue(value) {
  if (value == null) return null;
  const raw = value.toString().slice(1);
  try {
    const parsed = JSON.parse(raw);
    if (parsed !== null && typeof parsed === "object" && typeof parsed.value === "string" && parsed.value.trim() !== "") {
      return parsed.value;
    }
  } catch {}
...
function readTokenFromLeveldbDir(leveldbDir) {
  return new Promise((resolve) => {
    ...
    let db;
    try {
      db = leveldown(leveldbDir);
      // Use the low-level callback API directly: levelup wraps open errors in
      // an unhandled 'error' event, which crashes the host when a live browser
      // holds the DB. leveldown reports the same failure through open()'s
      // callback instead.
      db.open((openErr) => {
        if (openErr) return done(null);
        db.get(DEEPSEEK_STORAGE_KEY, (getErr, value) => {
          const token = getErr ? null : decodeLocalStorageValue(value);
          try { db.close(() => {}); } catch {}
          done(token);
        });
      });
...
async function copyLeveldbToTemp(srcDir) {
  const tmpDir = await mkdtemp(join(tmpdir(), "chicheng-ds-"));
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const children = await readdir(src, { withFileTypes: true }).catch(() => []);
      for (const child of children) {
        await copyFile(join(src, child.name), join(dest, child.name)).catch(() => {});
      }
    } else {
...
async function readPlatformTokenFromLeveldb(dir) {
  const direct = await readTokenFromLeveldbDir(dir);
  if (direct !== null) return direct;
  // If the browser is running and holds the LevelDB lock, read a temporary copy.
  const tmpDir = await copyLeveldbToTemp(dir).catch(() => null);
  if (tmpDir === null) return null;
  try {
    return await readTokenFromLeveldbDir(tmpDir);
  } finally {
    await rm(tmpDir, { recursive: true, force: true }).catch(() => {});
  }
}

async function importDeepseekPlatformToken() {
  const dirs = await browserProfileLeveldbDirs();
  for (const dir of dirs) {
    const token = await readPlatformTokenFromLeveldb(dir);
    if (token !== null) return { token, found: true, source: dir };
  }
...
function buildApi() {
  return {
    summary: async () => {
      ...
      const records = fold.allRecords();
      let deepseekInUse = false;
      for (let i = records.length - 1; i >= 0; i--) {
        const provider = records[i]?.provider;
        if (typeof provider === "string" && provider !== "" && provider !== "unknown") {
          deepseekInUse = /^deepseek(?:-|$)/i.test(provider);
          break;
        }
      }
      return {
    ...
    /** DeepSeek open-platform balance/consumption for the hover tooltip. */
    deepseek: async (payload) => {
      const now = Date.now();
      const force = payload?.force === true;
      if (!force && deepseekCache.data !== null && now - deepseekCache.at < DEEPSEEK_CACHE_TTL_MS) {
        return { ...deepseekCache.data, cached: true };
      }
      const data = await fetchDeepseekPlatformData();
      deepseekCache = { at: now, data };
      return data;
    },

    /** Whether DEEPSEEK_PLATFORM_TOKEN is currently configured (no value). */
    platformTokenStatus: async () => {
      const ctx = currentCtx;
      if (ctx?.credentials?.describe == null) return { configured: false, writable: false };
      const desc = await ctx.credentials.describe("DEEPSEEK_PLATFORM_TOKEN");
      return { configured: desc?.configured === true, writable: desc?.writable === true };
    },

    /** Scan local Chrome/Edge profiles for a stored DeepSeek platform userToken. */
    importPlatformToken: async () => importDeepseekPlatformToken(),

    /** Persist a DeepSeek platform userToken via the credentials seam. */
    savePlatformToken: async (payload) => {
      const token = typeof payload?.token === "string" ? payload.token.trim() : "";
      if (token === "") throw new StatsError("invalid-token", "UserToken 不能为空", 400);
      const ctx = currentCtx;
      if (ctx?.credentials?.set == null) throw new StatsError("credentials-unavailable", "credentials service unavailable", 500);
      await ctx.credentials.set("DEEPSEEK_PLATFORM_TOKEN", token);
      deepseekCache = { at: 0, data: null };
      return { saved: true };
    },
    ...
    usage: async (payload) => {
      ...
      const provider = typeof payload?.provider === "string" && payload.provider !== "" && payload.provider !== "all"
        ? payload.provider
        : "all";
      const from = rangeFromMs(range, now);
      const allRows = fold.queryRecords(from);
      // Provider breakdown over the FULL range (so the selector stays complete
      // while a single provider is being viewed).
      ...
      const providers = [...providerMap.entries()]
        .map(([name, entry]) => ({ provider: name, name: providerNameOf(name), requests: entry.requests, tokens: entry.tokens }))
        .sort((a, b) => b.tokens - a.tokens);
      const rows = provider === "all"
        ? allRows
        : allRows.filter((record) => (record.provider ?? "unknown") === provider);
      ...
      for (const record of rows) {
        ...
        entry.requests += 1;
        entry.tokens += record.input + record.cacheRead + record.cacheWrite + record.output;
        const sessionKey = record.session ?? "unknown";
        let sessionEntry = sessionMap.get(sessionKey);
        if (sessionEntry === undefined) {
          ...
          sessionMap.set(sessionKey, sessionEntry);
        }
        sessionEntry.requests += 1;
        sessionEntry.tokens += record.input + record.cacheRead + record.cacheWrite + record.output;
        sessionEntry.input += record.input;
        sessionEntry.cacheRead += record.cacheRead;
        sessionEntry.cacheWrite += record.cacheWrite;
        sessionEntry.output += record.output;
        if (Number.isFinite(record.duration)) {
          sessionEntry.durationSum += record.duration;
          sessionEntry.durationCount += 1;
        }
        if (record.t > sessionEntry.lastT) sessionEntry.lastT = record.t;
      }
      ...
      const sessions = [...sessionMap.entries()]
        .map(([session, entry]) => ({
          session,
          name: sessionTitles.get(session) ?? null,
          ...
          output: entry.output,
          avgDuration: entry.durationCount > 0 ? Math.round(entry.durationSum / entry.durationCount) : null,
          lastT: entry.lastT,
        }))
        .sort((a, b) => b.tokens - a.tokens || b.lastT - a.lastT)
        .map(({ lastT, ...rest }) => rest);
      ...
      for (const record of rows) {
        ...
        if (bucket === undefined) {
          bucket = { requests: 0, tokens: 0, input: 0, cacheRead: 0, cacheWrite: 0, output: 0 };
...
async function apply(ctx) {
  ...
  startupTimer = setTimeout(() => {
    ...
    void (async () => {
      ...
      await refreshSessionTitlesIfNeeded();
