...
function decodeSessionLog(buffer) {
  // Guard: the Node runtime bundled with DeepSeek Harness Desktop (0.1.5)
  // traps (SIGTRAP / EXC_BREAKPOINT) inside zlib.zstdDecompressSync on valid
  // zstd streaming frames that carry no Frame_Content_Size field — the exact
  // frame shape the harness session-log writer emits — so the startup sweep
  // killed the runtime process on every launch. Disable log backfill by
  // default (live event counting never touches zstd); opt back in with
  // CHICHENG_ENABLE_SCAN=1 once the runtime binding is fixed.
  if (process.env.CHICHENG_ENABLE_SCAN !== "1") return [];
