...
import { readFromStorage } from "inspect-browser-storage";
...
async function readPlatformTokenFromLeveldb(dir) {
  // Try without touching the browser's LOCK file first; if a live browser
  // holds the DB, fall back to the package's autoUnlink opening mode.
  try {
    const token = await readFromStorage("platform.deepseek.com", "userToken", {
      dbPath: dir,
      protocol: "https://",
      autoUnlink: false,
    });
    ...
    const message = error instanceof Error ? error.message : String(error);
    const locked = /lock/i.test(message) || /IO error/i.test(message);
  ...
  try {
    ...
    // profile not accessible / storage not present
  }
  return null;
}
