...
let currentCtx = null;
/** Latest client-side sidebar DOM outline (diagnostics; in-memory only). */
let lastDomReport = null;
...
function buildApi() {
  return {
    ...
    /** Client-side DOM diagnostic: the browser posts the sidebar outline here. */
    diag: async (payload) => {
      if (payload && typeof payload.dom === "string" && payload.dom.length > 0) {
        lastDomReport = { at: new Date().toISOString(), dom: payload.dom.slice(0, 12000) };
        console.info(`[chicheng-stats] dom report:\n${lastDomReport.dom}`);
      }
      return { report: lastDomReport };
    },
